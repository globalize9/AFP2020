...
def Get_candidate_pairs(data):
    ...
    companies = np.array(data.columns.get_level_values(0).unique()) #All the unique companies
    ...
    def TrendAdjPT(dataset, trend_data):
        last_peak_loc = df_1_test.index[np.where(df_1_test.Peak==1)[0][-1]].copy()
    ...
    def Days_above_below(dataset, trend_data):    
        ...
        df_1_test.loc[:,'Trend'] = trend_data.loc[:,'trend'].copy()
    ...
    for i in range(len(all_combinations)):    
        print(f'Currently Running 3: {i}')
        df_1= pd.DataFrame({'Index':[],
                            'PE_Ratio':[],
                            'Div_Diff':[]})

        ###### calculating the technical indicators
        df_1['Index'] = data[str(all_combinations[i].split('_')[0])]['PX_LAST']/data[str(all_combinations[i].split('_')[1])]['PX_LAST']
        df_1['PE_Ratio'] = data[str(all_combinations[i].split('_')[0])]['PE_RATIO']/data[str(all_combinations[i].split('_')[1])]['PE_RATIO']
        df_1['Div_Diff'] = (data[str(all_combinations[i].split('_')[0])]['AVERAGE_DIVIDEND_YIELD'] - data[str(all_combinations[i].split('_')[1])]['AVERAGE_DIVIDEND_YIELD'])
        ...
        indicators['Index'] = df_1_test.loc[end,'Index'].copy()
...
def top_10(candidate_pairs_overbought, candidate_pairs_candy_fish, candidate_pairs_rolled_over, candidate_pairs_crossed_over):
    ...
    top10_pairs = np.unique(top10_pairs)
    
    pd.DataFrame(top10_pairs, columns = ['Pairs']).to_csv("top10_list.csv")
...
def Factor_Cleaning_All(macro_factors_raw, feedstock_factors_raw):
    ...
    return([macro_factors, feedstock_factors])
...
def stepwise_factors(macro_factors, feedstock_factors, top10_pairs, price_data):
    ...
    for pair in range(len(top10_pairs)):
        print(f"Current pair 2 is: {pair}")
...
def Random_Forest_Code(top10_pairs, price_data, macro_factors, feedstock_factors, lag_factors):
...
# # Run Commodity Random Forest

# In[8]:


def Commodity_RF_Code(top10_pairs, price_data, macro_factors, feedstock_factors, lag_factors):
